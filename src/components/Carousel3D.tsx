...
const Carousel3D: React.FC = () => {
  ...
  const featuredPacks = useMemo(() => packs.filter(p => !['combo', 'complete'].includes(p.category)).slice(0, 6), []);
  ...
  return <div className="relative w-full max-w-6xl mx-auto mb-6 md:mb-8">
      ...
      perspective: '1000px'
        ...
        <AnimatePresence mode="wait">
          ...
          pack,
          position
        }, index) => <motion.div key={`${pack.id}-${position}`} initial={{
          opacity: 0,
          ...
          ease: "easeInOut",
          delay: index * 0.1
        }} className="absolute w-48 md:w-80 flex-shrink-0" style={{
      ...
      <div className="flex justify-center space-x-6 mb-6 md:mb-8 px-4">
        <button onClick={prevSlide} disabled={isLoading} className="bg-white text-black p-3 md:p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed z-20" aria-label="Pack anterior">
          <ChevronLeft className="h-5 w-5 md:h-6 md:w-6" />
        </button>
        <button onClick={nextSlide} disabled={isLoading} className="bg-white text-black p-3 md:p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed z-20" aria-label="Próximo pack">
      ...
      <div className="flex justify-center space-x-3 mt-6 md:mt-8 px-4">
        {featuredPacks.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`w-3 h-3 md:w-4 md:h-4 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-case-red' : 'bg-gray-600'}`} aria-label={`Ir para pack ${index + 1}`} />)}
      </div>
      ...
      <PaymentStatusModal isOpen={paymentStatus.isOpen} onClose={closePaymentStatus} status={paymentStatus.status} packName={paymentStatus.packName} />
